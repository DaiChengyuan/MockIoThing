...
public class ByteQueue implements Cloneable {
    ...
    public String toHexString() {
        ...
        StringBuffer sb = new StringBuffer();
        String headStr = Integer.toHexString(peek(0) & 0xff);
        if (headStr.length() == 1) {
            headStr = "0" + headStr;
        }
        sb.append(headStr);
        ...
        for (int i = 1; i < size; i++) {
            sb.append(' ');
            tempStr = Integer.toHexString(peek(i) & 0xff);
            if (tempStr.length() == 1) {
    ...
    public byte getXorChkSum() {
        byte x = 0;
        for (byte aByte : this.toHexBytes()) {
            x ^= aByte;
        }
        return x;
    }
